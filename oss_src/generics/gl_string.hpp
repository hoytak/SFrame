 ...
 */
#ifndef GRAPHLAB_GL_STRING_H_
#define GRAPHLAB_GL_STRING_H_
...
static inline
std::istream& getline(std::istream& is, graphlab::gl_string& str, char dlm='\n') {
  return graphlab::gl_string_internals::getline(is, str, dlm);
}
...
static inline int stoi(const graphlab::gl_string& str, size_t* idx = 0, int base = 10) {
  return std::stoi(std::string(str.begin(), str.end()), idx, base);
}

static inline long stol(const graphlab::gl_string& str, size_t* idx = 0, int base = 10) {
  return std::stol(std::string(str.begin(), str.end()), idx, base);
}
  
static inline unsigned long stoul (const graphlab::gl_string& str, size_t* idx = 0, int base = 10) {
  return std::stoul(std::string(str.begin(), str.end()), idx, base);
}

static inline unsigned long long stoull(const graphlab::gl_string& str, size_t* idx = 0, int base = 10) {
  return std::stoull(std::string(str.begin(), str.end()), idx, base);
}

static inline float stof (const graphlab::gl_string& str, size_t* idx = 0) {
  return std::stoll(std::string(str.begin(), str.end()), idx);
}

static inline double stod (const graphlab::gl_string& str, size_t* idx = 0) {
  return std::stod(std::string(str.begin(), str.end()), idx);
}
  
static inline long double stold(const graphlab::gl_string& str, size_t* idx = 0) { 
  return std::stold(std::string(str.begin(), str.end()), idx);
}

static inline void swap (graphlab::gl_string& a, graphlab::gl_string& b) noexcept {
  a.swap(b);
}
...
template <> struct hash<graphlab::gl_string>
  ...
  size_t operator()(const graphlab::gl_string& s) const {
    return graphlab::hash64(s.data(), s.size());
  }
};
