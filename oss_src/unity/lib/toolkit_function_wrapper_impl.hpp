...
#ifndef GRAPHLAB_TOOLKIT_FUNCTION_WRAPPER_IMPL_HPP
#define GRAPHLAB_TOOLKIT_FUNCTION_WRAPPER_IMPL_HPP
#include <type_traits>
  ...
  void operator()(boost::mpl::integral_c<int, n> t) const { 
    ...
    if (n < inargnames.size()) {
      boost::fusion::at_c<n>(*inargs) = 
          graphlab::safe_varmap_get<element_type>(*params, inargnames[n]);
    }
  }
};
...
generate_function_wrapper(Function fn, std::vector<std::string> inargnames) {
  ...
  auto fnwrapper =
      [fn, inargnames](variant_map_type args)->variant_type {
        graphlab::toolkit_function_response_type ret;
...
toolkit_function_specification make_spec(Function fn, std::string fnname,
  ...
  auto invoke_fn = [fnwrapper,inargnames](graphlab::toolkit_function_invocation& invoke)->graphlab::toolkit_function_response_type {
        graphlab::toolkit_function_response_type ret;
